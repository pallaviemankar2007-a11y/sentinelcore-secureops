...
export default function AssetsPage({
  ...
  // Self-managed state if props are not provided
  ...
  // Fetch data if external state is absent
  ...
  // Merge internal vs external props
  ...
  // Compute status counts dynamically if not provided
  ...
  const filtered = useMemo(() => {
    return assets
        .filter((a) => filter === 'all' || (a.status || 'HEALTHY') === filter)
        .filter((a) => (a.name || a.assetName || '').toLowerCase().includes(search.toLowerCase()))
  ...
  const handleMetricsSubmit = useCallback(async (values) => {
    ...
    try {
      await api.updateMetrics(metricsTarget.assetId || metricsTarget.id, values);
  ...
  return (
      <div style={{ padding: 24, background: '#0B0F19', minHeight: '100vh', color: '#FFF' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 18 }}>
          <div>
            <h1 style={{ fontSize: 22, fontWeight: 700, margin: 0 }}>Infrastructure Assets</h1>
            <p style={{ margin: 0, fontSize: 13, color: '#5B6684' }}>Monitor active servers, cloud instances, and networks.</p>
          </div>
            ...
            <div style={{ marginBottom: 20, borderRadius: 12, border: '1px solid rgba(255,255,255,0.06)', background: '#0F1830', padding: 18 }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
                ...
                <div style={{ display: 'flex', gap: 16, fontSize: 11, color: '#8B98B0' }}>
                  <LegendDot color="#2DD4BF" label={`${counts.HEALTHY || 0} healthy`} />
                  <LegendDot color="#F5A623" label={`${counts.WARNING || 0} warning`} />
                  <LegendDot color="#F0455D" label={`${counts.CRITICAL || 0} critical`} />
                </div>
              </div>
              <div style={{ display: 'flex', height: 56, alignItems: 'flex-end', gap: 3 }}>
                {assets.map((a, idx) => (
                    <div
                        key={a.assetId || a.id || idx}
                        title={`${a.name || a.assetName} · ${a.status || 'HEALTHY'}`}
                        style={{
                          flex: 1,
                          borderRadius: 2,
                          background: statusColor[a.status]?.dot || '#2DD4BF',
                          height: `${Math.min(Math.max(a.cpuUsage ?? 10, 8), 100)}%`,
                          opacity: a.status === 'CRITICAL' ? 1 : 0.75,
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                ...
                <tr style={{ background: '#0F1830', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
                  {['Asset', 'Type', 'Status', 'CPU', 'Memory', 'Disk', 'Network', 'Last checked', ''].map((h) => (
                      <th key={h} style={{ textAlign: 'left', padding: '10px 14px', fontSize: 10.5, textTransform: 'uppercase', letterSpacing: '0.05em', color: '#5B6684', fontWeight: 500 }}>
                        {h}
                      </th>
                  ))}
                </tr>
                ...
                {paginated.map((a, idx) => {
                  ...
                  const st = a.status || 'HEALTHY';
                  const c = statusColor[st] || statusColor.HEALTHY;
                  const assetIdStr = String(a.assetId || a.id || idx);

                  return (
                      <tr key={assetIdStr} style={{ borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                        <td style={{ padding: '11px 16px' }}>
                          <div className="mono" style={{ color: '#E2E8F0', fontWeight: 600 }}>{a.name || a.assetName}</div>
                          <div className="mono" style={{ fontSize: 10.5, color: '#3E4867' }}>{assetIdStr.slice(0, 8)}</div>
                        </td>
                        <td style={{ padding: '11px 16px' }}>
                          <div style={{ display: 'flex', alignItems: 'center', gap: 6, color: '#8B98B0' }}>
                            <Icon size={13} /> {a.type}
                          </div>
                        </td>
                        <td style={{ padding: '11px 16px' }}>
                      <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '3px 9px', borderRadius: 99, fontSize: 11, fontWeight: 500, color: c.text, background: c.bg }}>
                        <span style={{ width: 6, height: 6, borderRadius: '50%', background: c.dot }} /> {st}
                      </span>
                        </td>
                        ...
                        <td style={{ padding: '11px 14px' }}>
                          <div style={{ display: 'flex', gap: 4, justifyContent: 'flex-end' }}>
                            {MetricsModal && <IconButton onClick={() => setMetricsTarget(a)} title="Update metrics"><Zap size={13} /></IconButton>}
  ...
  return (
      ...
      <span style={{ fontSize: 12.5, color: '#8B98B0' }}>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          <button onClick={() => onChange(Math.max(1, page - 1))} disabled={page === 1} style={navBtnStyle(page === 1)}>
            ‹ Prev
          </button>
          <button onClick={() => onChange(Math.min(totalPages, page + 1))} disabled={page === totalPages} style={navBtnStyle(page === totalPages)}>
            Next ›
          </button>
        </div>
      </div>
...
function SkeletonTable() {
  return (
      <div style={{ borderRadius: 12, border: '1px solid rgba(255,255,255,0.06)', padding: 16 }}>
        {[...Array(5)].map((_, i) => (
            <div key={i} style={{ height: 36, borderRadius: 6, background: 'rgba(255,255,255,0.03)', marginBottom: 8 }} />
        ))}
      </div>
  );
}
